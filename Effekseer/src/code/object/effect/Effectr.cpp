...
bool My::Effect::Load(const std::string Filepas, D3DXVECTOR3 pos)
{
	...
	//ファイルの存在確認
	//std::ifstream ifs(Filepas);
	//if (!ifs.is_open()) {
	//	std::cerr << "Error: File not found at path: " << Filepas << "\n";
	//	return false;
	//}
	...
	if (m_effectRef == nullptr) {
		...
		oss << "Effect::Load : ファイルパスの場所に指定のデータはありませんでした" << Filepas << "\n";
	...
	m_handle = managerRef->Play(m_effectRef, pos.x, pos.y, pos.z);
	return true;
}
/// <summary>
/// 色取得
...
/// </summary>
/// <param name="scl">設定するスケール</param>
void My::Effect::SetScl(float scl)
{
	// マネージャーRef取得
	My::CManager* pManager = CManager::GetInstance();   // マネージャー取得
	My::CEffekseerManager* pEffekseerManager = pManager->GetEffekseerManager(); // エフェクシアマネージャー取得
...
void My::Effect::SetColorSRGB(Effekseer::Color col)
{
	// マネージャーRef取得
	My::CManager* pManager = CManager::GetInstance();   // マネージャー取得
	My::CEffekseerManager* pEffekseerManager = pManager->GetEffekseerManager(); // エフェクシアマネージャー取得
...
/// </summary>
/// <param name="scl">加算するスケール</param>
void My::Effect::AddScl(float scl)
{
	// マネージャーRef取得
	My::CManager* pManager = CManager::GetInstance();   // マネージャー取得
	My::CEffekseerManager* pEffekseerManager = pManager->GetEffekseerManager(); // エフェクシアマネージャー取得
...
void My::Effect::AddColor(D3DXCOLOR col)
{
	// マネージャーRef取得
	My::CManager* pManager = CManager::GetInstance();   // マネージャー取得
	My::CEffekseerManager* pEffekseerManager = pManager->GetEffekseerManager(); // エフェクシアマネージャー取得
...
void My::Effect::AddColorSRGB(const std::array<int, 4>& col)
{
	// マネージャーRef取得
	My::CManager* pManager = CManager::GetInstance();   // マネージャー取得
	My::CEffekseerManager* pEffekseerManager = pManager->GetEffekseerManager(); // エフェクシアマネージャー取得
...
My::Effect* My::Effect::create(const std::string sFilepas)
{
	...
	m_pEffect->Load(sFilepas, m_pEffect->paramater.m_scl);
...
Effekseer::Color My::Effect::ColorFloatToSRGB(const D3DXCOLOR col)
{
...
/// <returns>変換した色</returns>
D3DXCOLOR My::Effect::ColorSRGBToFloat(const Effekseer::Color col)
{
