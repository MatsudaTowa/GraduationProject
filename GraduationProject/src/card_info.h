...
namespace My
{
	...
	struct IHttpClient {
		...
		virtual std::future<HttpResponse>GetAsync(const std::string& url, const std::vector<std::pair<std::string, std::string>>& headers = {}, int timeoutMs = 10000) = 0;

		virtual std::future<HttpResponse>PostAsync(const std::string& url, const std::vector<uint8_t>& body, const std::string& contentType, const std::vector<std::pair<std::string, std::string>>& headers = {}, int timeoutMs = 10000) = 0;
	...
	*/
	...
	public:
		...
		*/
		...
		// IHttpClient を注入するためのセッター
		void SetHttpClient(const std::shared_ptr<IHttpClient>& client);
	private:
		BaseParam m_Param;
		CardRecord m_Record;
		std::shared_ptr<IHttpClient> m_Client; 
		std::mutex m_ClientMutex; // マルチスレッドで使うなら保護
	};
}
#endif