...
namespace My
{
	...
	*/
	...
	public:
		...
		* @return 成功したかどうか
		*/
		HRESULT LoadFile(const std::string& path, const std::string token);

	private:
		/** @brief ファイルをバイナリで読み込みバッファに格納する */
		HRESULT readFileToBuffer(const std::string& path, std::vector<uint8_t>& outBuf) const;

		/** @brief バッファが CBOR かどうかを判定する */
		bool isCborBuffer(const std::vector<uint8_t>& buf) const;

		/** @brief JSON を CardRecord に変換する */
		CardRecord FromJsonToRecord(const nlohmann::json& j) const;

		/** @brief CardRecord の妥当性検査 */
		bool Validate(const CardRecord& rec, std::string* outError = nullptr) const;

		/** @brief 警告ログ出力（簡易） */
		void logWarn(const std::string& tag, const std::string& msg) const;
