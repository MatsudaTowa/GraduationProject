...
namespace My
{
	...
	enum AddEffect
	{
		EFFECT_NONE = 0,	// 効果追加無し
		EFFECT_DAMAGE,		// ダメージ
		EFFECT_ENERGY,		// エナジー
		EFFECT_DRAW,		// 山札をドローする
	};
	...
	struct EFFECTENERGY
	{
		...
		enum target
		{
			NONE_TARGET,		// 初期化
			TARGET_ONLYME,		// 自分だけ
			TARGET_SPECIFIC,	// 特定の相手
			TARGET_RANDOM,		// ランダム
			TARGET_SELFIN,		// 自分を含めてランダム
			TARGET_ALL,			// 全体
		};

		int num = 1;			// 数値
		int selecttarget = -1;	// ターゲット(自分だけ/特定の相手/自分を含めずランダム/ 自分を含めてランダム/全体)
		int member = 1;			// 人数(ランダムの場合に設定)
		int ope = -1;			// 演算子(+-*/)
	};
	...
	struct EFFECTDAMAGE
	{
		...
		enum target
		{
			...
			TARGET_ONLYME,		// 自分だけ
			TARGET_SPECIFIC,	// 特定の相手
			TARGET_RANDOM,		// 自分を含めないでランダム
			TARGET_SELFIN,		// 自分を含めてランダム
			TARGET_ALL,			// 全体
		};

		int selecttarget = -1;	// 対象(自分だけ/特定の相手/自分を含めないでランダム/自分を含めてランダム/全体)
		int member = 1;			// ランダムの場合人数設定
		int num = 1;			// 数値
	};
	...
	struct EffectEntry
	{
		AddEffect type = EFFECT_NONE;
		EFFECTENERGY energy;	// エナジー用
		EFFECTDAMAGE damage;	// ダメージ用
		EFFECTDRAW draw;		// ドロー用
		EffectEntry() = default;
		explicit EffectEntry(AddEffect t) : type(t){}
	};
	...
	struct Card
	{
		...
		int editingEffectIndex = -1;				// 効果追加の編集時に選択している効果
		...
		EFFECTENERGY effectenergy;	// エナジーの構造体
		EFFECTDAMAGE effectdamage;	// ダメージの構造体
		EFFECTDRAW effectdraw;		// ドローの構造体

		bool addeffect = false;	// 効果追加編集画面表示用フラグ

		std::vector<Reference> references; // 型安全な参照配列
		std::vector<EffectEntry>effects;	// 効果追加配列
	};
	...
	public:
		...
		* @param [in]カード番号
		*/
		void SetEffect(int PackID, int CardID);
